...
class Editor:
    def __init__(self, scale_horizontal, scale_vertical, screen, font):
        ...
        self.lines = [""]
        self.curr_line = 0
        self.lines_shown = [0, round((920 * self.scale_vertical) / (40 * self.scale_vertical)) - 1]
    ...
    def draw(self):
        ...
        if len(self.lines) != 0:
            ...
            for i in range(len(self.lines)):
                txt = f"{self.lines_shown[0] + i}: " + self.lines[i]
                if i == self.curr_line:
                    txt += "_"
                txt_render.append(self.font.render(txt, 1, WHITE))
                
            txt_render = txt_render[self.lines_shown[0]:self.lines_shown[1] + 1]
    ...
    def click_check(self, event_pos):
        ...
        if not self.save_file and not self.open_file:  
            ...
            if not self.save_file_hitbox.collidepoint(event_pos):
                self.save_file = False
            if self.open_file:  
                for i in self.open_buttons[0:-1]:
                ...
                if self.open_buttons[-1].is_pressed(event_pos):
                    self.lines = [""]
                    amount_to_draw = round((970 * self.scale_vertical) / (40 * self.scale_vertical)) - 1
                    self.lines_shown[0] = 0
                    self.lines_shown[1] = amount_to_draw
